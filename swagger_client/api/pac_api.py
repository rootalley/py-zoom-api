...
class PACApi(object):
    ...
    def user_pa_cs(self, user_id, **kwargs):  # noqa: E501
        ...
        if kwargs.get('async_req'):
            return self.user_pa_cs_with_http_info(user_id, **kwargs)  # noqa: E501
        else:
            (data) = self.user_pa_cs_with_http_info(user_id, **kwargs)  # noqa: E501
            return data

    def user_pa_cs_with_http_info(self, user_id, **kwargs):  # noqa: E501
        """List a User's PAC Accounts  # noqa: E501

        [Personal Audio Conference](https://support.zoom.us/hc/en-us/articles/204517069-Getting-Started-with-Personal-Audio-Conference) (PAC) allows Pro or higher account holders to host meetings through PSTN (phone dial-in) only.<br>Use this API to list a user's PAC accounts.<br><br> **Scopes:** `user:read:admin` `user:read`<br> <br>  **[Rate Limit Label](https://marketplace.zoom.us/docs/api-reference/rate-limits#rate-limits):** `Light`<br> **Prerequisites:**<br> * A Pro or higher plan with [Premium Audio Conferencing](https://support.zoom.us/hc/en-us/articles/204517069-Getting-Started-with-Personal-Audio-Conference) add-on. * Personal Audio Conference must be enabled in the user's profile.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.user_pa_cs_with_http_info(user_id, async_req=True)
        >>> result = thread.get()
        ...
        if ('user_id' not in params or
            ...
            raise ValueError("Missing the required parameter `user_id` when calling `user_pa_cs`")  # noqa: E501
